...
React Native existante avec des screenshots prêts. Objectif : la montrer dans le
portfolio avec un impact visuel fort, sans déséquilibrer le positionnement ni casser
la cohérence ou les Core Web Vitals soignés à l'Epic 4.
...
- **3 screenshots** présentés dans un **carrousel**.
...
```ts
screens?: string[]      // ex. ["/images/projects/mobile-app/screen-1.webp", ...]
...
Screenshots → `public/images/projects/mobile-app/screen-1.webp`,
`screen-2.webp`, `screen-3.webp` (convention `.webp` cohérente avec les autres
projets, optimisée pour les Core Web Vitals).

## Tests (Vitest + Testing Library)

- `PhoneMockup.test.tsx` : rend bien 3 écrans ; la navigation préc/suiv change
...
## Entrées à fournir par l'utilisateur (au moment de l'implémentation)

- Les 3 fichiers screenshots déposés dans `public/images/projects/mobile-app/`.
- Le **nom de l'app** et **une phrase** de description (titre + `description` /
  `longDescription` de la carte).
- La `techStack` de l'app (ex. React Native, Expo, TypeScript…).
